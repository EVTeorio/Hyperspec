...
# Function to split ROI polygons and create new polygons with a buffer
split_roi <- function(roi, buffer_distance = 1) {
  ...
  # Create a buffer around the split line
  buffer_area <- st_buffer(split_line, dist = buffer_distance)
  
  # Use st_difference to create new polygons
  left_polygon <- st_difference(roi, buffer_area)
  right_polygon <- st_difference(roi, buffer_area)
  
  # Validate and adjust
  if (st_is_empty(left_polygon)) {
    left_polygon <- st_union(roi)  # Fallback to the original if left is empty
  }
  
  if (st_is_empty(right_polygon)) {
    right_polygon <- st_union(roi)  # Fallback to the original if right is empty
  }
  
  # Return the two new polygons
  return(list(left_polygon = st_make_valid(left_polygon), right_polygon = st_make_valid(right_polygon)))
}
...
for (i in seq_along(roi_polygons)) {
  split_rois <- split_roi(roi_polygons[[i]], buffer_distance = 1)  # Adjust buffer distance as needed
  
  # Get the original name of the current ROI
  original_name <- xml_attr(regions[[i]], "name")
  
  # Ensure split_rois has valid parts
  if (length(split_rois) == 0 || st_is_empty(split_rois$left_polygon) || st_is_empty(split_rois$right_polygon)) {
    warning(paste("No valid split for ROI:", original_name))
    next
  }
  
  # Add left and right parts to new ROIs
  new_rois[[length(new_rois) + 1]] <- list(polygon = split_rois$left_polygon, name = paste0("1_", original_name))
  new_rois[[length(new_rois) + 1]] <- list(polygon = split_rois$right_polygon, name = paste0("2_", original_name))
}
